...
export const ContestPage = () => {
  ...
  const [isLogin, setIsLogin] = useState<boolean>(location.pathname.split('/').length === 5 ? true : false)
  ...
  const handleJoinContest = async () => {
    try {
      ...
      if (res.status === 200) {
        ...
        setIsLogin(true)
        getContestUserInfo()
        navigate(`${location.pathname}/${userName}`, { state: location.state })
      }
    } catch (err) {
      console.log(err)
    }
  ...
  return (
    <div style={{ margin: '20px' }}>
      <h1>
        {location.state.contest_name}
        {('000' + String(location.state.time - 1)).slice(-3)}
      </h1>
      <p>
        {`${location.state.start_date.getHours().toFixed().padStart(2, '0')}:${location.state.start_date
          .getMinutes()
          .toFixed()
          .padStart(2, '0')}`}{' '}
        ~{' '}
        {`${location.state.end_date.getHours().toFixed().padStart(2, '0')}:${location.state.end_date
          .getMinutes()
          .toFixed()
          .padStart(2, '0')}`}{' '}
        ({(location.state.end_date.getTime() - location.state.start_date.getTime()) / 1000 / 60}分)
      </p>
      {isLogin === false && (
              ...
              return (
                <th key={index}>
                  <a href={problemUrl} target="_blank" rel="noreferrer">
                    {String.fromCharCode(Number('A'.codePointAt(0)) + index)}
                  </a>
                </th>
              )
            })}
